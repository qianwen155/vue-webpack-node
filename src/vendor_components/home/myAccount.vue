...
<script>
  import {getAccountMsg, getVendorVouchers} from '@/vendor_api/home'
  import swal from 'sweetalert2'
  // import { eventHub } from '@/utils/eventHub'
  export default {
    name: 'vendor_myAccount',
    data () {
      return {
        sign: '',
        pagesize: 20,
...
//          console.log(res)
          ...
          if (res.data.code === 412) {
            swal({
              ...
              if (result.value) {
                this.$router.push('/vendor-completeProfile')
              }
            })
          } else {
            swal({
    ...
    methods: {
      ...
      totransfer () {
        this.$router.push('/vendor-transfer')
      },
      tocheck () {
        this.$router.push('/vendor-checking')
      },
      tocarryover () {
        this.$router.push('/vendor-carryover')
      },
      toDetail (type, vid) {
        if (type === 'WITHDRAWAL') {
      ...
      getVendorVouchers () {
        getVendorVouchers(this.page).then(res => {

  ...
  <div v-cloak >
    <ul class="">
        <li class="list"  v-for="(record,index) in transferArr" :key="index" @click="toDetail(record.voucherId)">
            <div class="status">
                <span v-bind:style="{'color': statusColor[record.statuss]}">{{record.status | statusFilter}}</span>
              <div class="icon1"></div>
            </div>
            <div class="msg">
...
<script>
import {getTransferList} from '@/vendor_api/transfer'
import swal from 'sweetalert2'
export default {
  name: 'vendor_transferlist',
  data () {
    return {
      sign: '',
      transferArr: [],

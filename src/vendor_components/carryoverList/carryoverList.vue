  ...
  <div v-cloak >
    <ul class="">
        <li class="list" v-for="(record,index) in transferArr" :key="index" @click="toDetail(record.voucherId)">
            <div class="status">
                <span style="color:#01a403">转账成功</span>
              <div class="icon1"></div>
            </div>
            <div class="msg">
...
<script>
import {getCarryoverList} from '@/vendor_api/carryover'
import swal from 'sweetalert2'
export default {
  name: 'vendor_carryoverlist',
  data () {
    return {
      sign: '',
      transferArr: [],
  ...
  methods: {
    getCarryoverList () {
      getCarryoverList(this.page, 'TRANSFER').then(res => {
        console.log(res)

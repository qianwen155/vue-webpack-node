...
function getAccountMsg () {
  return fetch({
    url: 'v1/accounts-vendor/withdrawal?',
    method: 'get',
    timeout: 5000
  })
}
...
function postCarryover (data) {
  return fetch({
    url: 'v1/accounts-vendor/transfer',
    method: 'post',
    timeout: 5000,
    data: data
  })
...
function getCarryoverList (page, type) {
  return fetch({
    url: 'v1/accounts-vendor/voucher-history?pagesize=10&page=' + page + '&type=' + type,
    method: 'get',
    timeout: 5000
  })
}
...
function getCarryoverDetail (vid) {
  return fetch({
    url: 'v1/accounts-vendor/vouchers/' + vid,
    method: 'get',
    timeout: 5000
  })
}
function getAccountList (contain) {
  return fetch({
    url: 'v1/accounts-vendor/transfer?contain=' + contain,
    method: 'get',
    timeout: 5000
  })
}
